 ...
 * @param {Map<string, number>} [args.reachable]  Cells the active actor can move to.
 ...
 * @returns {HTMLElement}
 */
export function renderBoard({ tile, actors = [], reachable, targets, allies, active, onCell }) {
  ...
  for (let y = 0; y < tile.h; y++) {
    for (let x = 0; x < tile.w; x++) {
      board.append(renderCell({ tile, x, y, actors, reachable, targets, allies, active, onCell }));
    }
  }
  return el('div.board-wrap', {}, [board]);
}

function renderCell({ tile, x, y, actors, reachable, targets, allies, active, onCell }) {
  const key = `${x},${y}`;
  const code = cellAt(tile, x, y);
  const occupant = actors.find((a) => a.alive && a.x === x && a.y === y);
  ...
  const interactive = Boolean(
    onCell && code !== WALL && (reachable?.has(key) || targets?.has(key) || allies?.has(key)),
  );
  ...
  const cell = el(interactive ? 'button' : 'div', {
    ...
    'aria-label': describeCell(code, occupant, x, y),
...
/** @param {string} code @param {Actor | undefined} occupant */
function describeCell(code, occupant, x, y) {
