...
- Canonical `d6` stat profiles plus deterministic `d100` derivation with per-entity overrides
- Two rule resolvers behind one contract (`d6`, `d100`)
...
- [ ] The same seed reproduces the same delve in both rule systems
- [ ] A full delve is completable in portrait orientation on a phone, using taps only
- [ ] Rule system is switchable and both are playable start to finish
...
| Risk | Mitigation |
|------|------------|
| Two rule systems, neither balanced | Single canonical profile plus derivation; `d6` is the balance reference; simulation reports each system separately |
