...
| D-35 | **Engagement and advantage are baseline combat rules** | Any movement step begun adjacent to an enemy provokes one reaction strike from that enemy unless an ability permits safe disengage; each enemy reacts at most once per move. Surrounding (2+ adjacent enemies) and kill momentum grant attack modifiers, and Slayer gains a capped melee modifier from being surrounded |
